...
**คำตอบ:** เพราะ max ถูกส่งเข้า clamp() แบบ pass by value ทำให้ clamp() ได้สำเนาของค่า max และการเปลี่ยนค่า max ภายในฟังก์ชันไม่เปลี่ยนค่า max ใน main()

### 2.2 `lerp` — Local Scope

**คำถาม:** ทำไม `main()` ถึงมองไม่เห็นตัวแปร `result` ทั้งที่มันถูกประกาศไว้ในไฟล์เดียวกัน?

**คำตอบ:** เพราะ result เป็น local valuable ของ lerp
...
**คำตอบ:** เพราะ min ส่งเข้า randomrange() แบบ pass by value ค่าที่อยู่ใน randomrange() เป็นสำเนาของ min จาก main() ถ้าเปลี่ยน min ภายในฟังก์ชัน จะไม่ส่งผลต่อ min ใน main()
...
**คำตอบ:** เพราะ current  และ  total ถูกส่งแบบ pass by value เป็นสำเนาฟังก์ชัน main() ซึ่งไม่ส่งผลกับตัวแปรเดิม
